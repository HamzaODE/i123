...
const StepFifteen = ({ routeHandler }) => {
  const [intellectualPropertyComplaint, setIntellectualPropertyComplaint] = useLocalSave("intellectualPropertyComplaint", "");

  const submitHandler = (e, selection) => {
    e.preventDefault();
    setIntellectualPropertyComplaint(selection);
